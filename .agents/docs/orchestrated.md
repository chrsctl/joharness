...
| looping | `LOOP?` — one file rewritten `JOHARNESS_CHURN_LIMIT`+ times; or head moved on three passes with `next:` unchanged | ALIVE, confirmed on the pair: `updated_at` and head both moved between the last two passes | kill with the record, respawn one tier up. Not alive on that pair = a dead manager wearing this shape; it wants a successor, not a tier escalation (`.claude/commands/orchestrate.md`, the LOOP row and its worked reading) |
