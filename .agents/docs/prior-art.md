...
### Taken, for orchestrated mode

Re-read at the same commit on 2026-09-05 when the requester asked for an
orchestrated mode ([`orchestrated.md`](orchestrated.md)). What crossed,
and the shape it took:

- **Coordinator and monitor.** Its Mayor dispatches; its Witness
  "detects stuck agents, triggers recovery (nudge or handoff), manages
  session cleanup" (`README.md`); its Deacon patrols across rigs. Here one
  orchestrator role, two steps of one pass — health, then spawn — because
  an in-repo harness has no daemon to hold a Witness, and a second session
  watching the first is a second thing to watch.
- **Nudge before kill; handoff makes the cycle survivable.** "Session
  cycling is normal operation, not failure" (`docs/concepts/polecat-lifecycle.md`),
  and the work survives because the sandbox and the hook persist. Here the
  branch persists and the workstream file is the hook; the orchestrator
  kills only after that file is written, by the manager or by itself.
- **Health vocabulary.** Working, idle, done, stalled, zombie
  (`polecat-lifecycle.md`). Here working, stalled, gone, blocked, done —
  `blocked` because this harness has a state Gas Town's table lacks, a
  session that stopped on purpose for a human, and it must never be
  respawned.
- **A cap.** `scheduler.max_polecats`, added because "slinging N beads
  spawns N polecats simultaneously, exhausting API rate limits"
  (`docs/design/scheduler.md`). Here `JOHARNESS_MAX_MANAGERS`, the human's.
- **GUPP.** Already convergent above; the spawn prompt is the hook.

### Not taken, same reasons as above

The ledger, the merge queue, the seance and integration branches keep
their rejections. Its persistent worker pool with names and CVs
(`docs/design/persistent-polecat-pool.md`) is not taken either: identity
here is the branch and the workstream file, attribution is the commit, and
a pool is state outside git.
