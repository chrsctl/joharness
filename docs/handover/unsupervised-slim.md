...
- Reconciled 2026-09-02 with PR `simplify-unsupervised-mode` (`main`
  ...
  1-3 stand as the second cut and now name that tree. Its r4 (the stop
  condition moved without a flag to the human) was flagged as product
  direction — same reason this plan's merge is the human's.
...
Round 3, self, after `main` moved (merge 7cc2409):

- r20: Scope named code the merge had already removed
  (`src_stop_condition`, `JOHARNESS_IN_SWEEP`, `--prev-dry`,
  `qc_edge_unsupervised`, `qc_goal_reached`, `drain_goal_reached`,
  `perf_count`'s export) and missed what it added (`drain_wave1`, the
  hook's EXIT trap, the unsupervised edge line, the "two stops" pointer in
  `handover-context.sh`, the conf comment). (fixed: Scope rewritten
  against 15c5df8, every symbol re-grepped)
- r21: baseline sizes, `cmd_sources` and `cmd_drain` counts were 4cae61a's.
  (fixed: re-counted on 15c5df8 — 5624 / 831 / 9697; 122; 152 — commands
  beside them)
- r22: the acceptance grep would hit the Runs table the merge moved into
  `.agents/docs/unsupervised.md`, which quotes the marking twice as
  history. (fixed: that file is checked outside its Runs table)
- r23: the requirement's deleted-bullet phrases were the old file's; the
  merge rewrote every bullet. (fixed: phrases re-measured, one hit each)
- r24: with `drain` now the ordering reader, deleting the wave-1 order
  left unsupervised with no fan-out at all. (fixed: one spawn line in
