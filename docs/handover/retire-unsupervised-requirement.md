...
- r6: (verifier) the retirement does not meet the lifecycle it cites: no
  ...
  the queue said decompose rather than delete. (fixed as filed, with one
  caveat named rather than closed. The process half: this plan now declares
  `requirement: unsupervised-mode`, so the retirement runs on the mechanism
  the README actually names — the last plan's pull request deleting the
  requirement with the plan file — instead of on this file's prose. The
  queue's "decompose" is answered too: this plan IS the decomposition, it
  just failed to declare the edge. What remains is not a process defect but
  a judgment: the endurance bullet reads NOT shown, so "satisfied" means
  the harness delivered everything it can rather than every bullet coming
  true. That is the requester's to accept, it is stated plainly in the pull
  request rather than assumed, the bullet is rehoused verbatim in issue 165
  by r7, and one `git revert` puts the file back)
