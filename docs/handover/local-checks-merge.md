...
- r14: (verifier, clean) no unquoted expansions, no `grep -q` SIGPIPE, no
  ...
  runner. (no change needed — recorded because a clean pass is a finding
  too, and because the marker gate is right that a bullet with no verdict is
  a silent drop whatever its content.)
