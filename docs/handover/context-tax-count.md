...
- r6: (session, clean) `verify` on this branch first ran 2 passed / 4 failed.
  ...
  provisioning order. (no change needed — the diff touches nothing under
  `.agents/env/`, and the layer is green on this branch)
...
- r18: (ci) `== finding verdicts` redded the retire commit: r6 was written as
  a clean-pass note and carried no verdict, and the gate is right that a
  retired file has no later moment to disposition one. A clean pass is still
  a finding and still owes a word. (fixed — r6 says no change needed, and
  why)
