...
- r10: (session) the perf-budget red recorded on the previous branch as
  ...
  record now sits in merged history and cannot be edited (no change — the
  claim it corrects merged with PR 183)
- r11: (ci) r10 was written `(no action ...)`, which `fb_marker` does not
  accept; the gate redded the branch and said why it had to red NOW —
  the file is retired, so there is no later moment to disposition it in.
  Caught after the retire commit, so the file was restored and re-retired,
  which the protocol anticipates (`.agents/docs/handover/README.md`:
  a workstream retired, restored to record a late finding, and retired
  again has two delete commits) (fixed)
- r12: (session) `main` moved 4 commits mid-branch (PR 184, research nodes
  open with a frontmatter block). Merged in rather than finishing behind;
  it changes what counts as a node in `docs/research/`, not what a
  graduation owes, and the node this branch deletes carried frontmatter
  (no change)
