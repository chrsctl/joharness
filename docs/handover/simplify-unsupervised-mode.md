...
- r4: (verifier) the stop condition moved from the ratified two-sweeps, no-open-pull-request form without a flag to the human. (wontfix on this branch — product direction, decided and written down per Decide alone; flagged in the pull request and the closing report, the human reverts the bullet if the ratified form is wanted)
