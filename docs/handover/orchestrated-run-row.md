...
- r12: (verifier, second pass on the committed tree) confirmed ten of ten fixed, recomputed 4863.78 + 388.64 = 5252.42 and both windows (42h20m, 82h40m) independently, and found the r11 drift on its own from the plan file — the same defect two readers reached by different routes, one grepping shared numbers across files and one recomputing from the anchors. It also saw the working tree dirty mid-review and correctly refused to credit an uncommitted "(fixed)"; that fix is `a070efc`. No new defect. Table row: 7 cells against a 7-column header, `Ended by` honest for a run that has not ended.
