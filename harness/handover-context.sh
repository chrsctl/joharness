...
while IFS= read -r ref; do
  ...
  # 'origin/<branch>' alone reports the session's own branch back at it from
  # any second remote — which is every contributor working from a fork, the
  # only option without push access. A same-named branch on another remote is
  # the same workstream; that is what the naming convention means.
  name="${short#*/}"
  [ "$name" = "HEAD" ] && continue
  ...
  # silently drops real branches off the end. Where origin has the name, that
  # is the entry to show; a checkout with no origin loses nothing.
  if [ "${short%%/*}" != "origin" ] &&
